				...
				<div v-if="settingsStore.enableSalesPersons" class="bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-200 rounded-lg p-3">
					...
					<div class="relative mb-2">
						<input
							...
							class="w-full px-3 py-2 ps-9 pe-8 text-xs border border-purple-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent bg-white"
						...
						<button
							@click="refreshSalesPersons"
							:disabled="loadingSalesPersons"
							class="absolute end-2 top-1/2 -translate-y-1/2 text-purple-500 hover:text-purple-700 disabled:opacity-40 transition-colors"
							:title="__('Refresh')"
						>
							<svg :class="['w-4 h-4', loadingSalesPersons ? 'animate-spin' : '']" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
							</svg>
						</button>
					</div>
