	...
	>
		<template #body-content>
			...
			<div v-else-if="invoiceData" class="flex flex-col gap-6">
				...
				<div>
					...
					<div class="md:hidden flex flex-col gap-3">
						...
						>
							...
							<div v-if="item.serial_no || item.custom_insurance_sr_no" class="mt-2 pt-2 border-t border-gray-100 text-start space-y-1">
								<div v-if="item.serial_no" class="flex items-center gap-1 text-xs">
									<svg class="w-3.5 h-3.5 text-blue-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
										<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14"/>
									</svg>
									<span class="text-gray-500">{{ __('Serial:') }}</span>
									<span class="font-medium text-gray-700">{{ item.serial_no }}</span>
								</div>
								<div v-if="item.custom_insurance_sr_no" class="flex items-center gap-1 text-xs">
									...
									<span class="text-gray-500">{{ __('Insurance:') }}</span>
									<span class="font-medium text-green-700">{{ item.custom_insurance_sr_no }}</span>
								</div>
							</div>
						</div>
					</div>
						...
						<table class="min-w-full divide-y divide-gray-200">
							<thead class="bg-gray-50">
								<tr>
									...
									<th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">{{ __('Serial No') }}</th>
							...
							<tbody class="bg-white divide-y divide-gray-200">
								<tr v-for="(item, idx) in invoiceData.items" :key="idx" class="hover:bg-gray-50">
									...
									<td class="px-4 py-3 text-center text-sm">
										<div v-if="item.serial_no || item.custom_insurance_sr_no" class="space-y-1">
											<div v-if="item.serial_no" class="flex items-center justify-center gap-1">
												<svg class="w-3.5 h-3.5 text-blue-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
													<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14"/>
												</svg>
												<span class="text-gray-700">{{ item.serial_no }}</span>
											</div>
											<div v-if="item.custom_insurance_sr_no" class="flex items-center justify-center gap-1">
												<svg class="w-3.5 h-3.5 text-green-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
													<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/>
												</svg>
												<span class="text-green-700">{{ item.custom_insurance_sr_no }}</span>
											</div>
										</div>
				...
				<div v-if="invoiceData.sales_team && invoiceData.sales_team.length > 0">
					<h4 class="text-sm font-semibold text-gray-700 mb-3 flex items-center">
						<svg class="w-4 h-4 me-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					...
					<div class="flex flex-wrap gap-2">
						<div
							v-for="(member, idx) in invoiceData.sales_team"
							:key="idx"
							class="flex items-center gap-2 px-3 py-2 bg-purple-50 border border-purple-200 rounded-lg"
						>
							<div class="w-7 h-7 rounded-full bg-purple-200 flex items-center justify-center">
								<svg class="w-4 h-4 text-purple-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
								</svg>
							</div>
							<div>
								<div class="text-sm font-medium text-gray-900">{{ member.sales_person }}</div>
								<div class="text-xs text-purple-600 font-semibold">{{ member.allocated_percentage }}%</div>
							</div>
						</div>
					</div>
				</div>
