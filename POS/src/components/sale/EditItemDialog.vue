	...
	<Dialog v-model="show" :options="{ title: __('Edit Item Details'), size: 'md' }">
		...
		<template #body-content>
			<div v-if="localItem" class="flex flex-col gap-4">
				...
				<!-- Insurance Serial Number Section (only for insurance items) -->
				<div v-if="localItem?.requires_insurance_sr_no" class="border-t border-gray-200 pt-4">
					<div class="flex items-center justify-between mb-3">
						<label class="block text-sm font-medium text-gray-700 text-start">
							{{ __('Insurance Serial Number') }}
							<span v-if="localInsuranceSerialNo" class="ms-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
								{{ __('Entered') }}
							</span>
						</label>
					</div>
					<div class="relative">
						<input
							v-model="localInsuranceSerialNo"
							type="text"
							:placeholder="__('Enter insurance serial number...')"
							class="w-full px-3 py-2 ps-10 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
						/>
...
watch(
	...
	(newItem) => {
		if (newItem) {
			...
			// Initialize insurance serial number
			localInsuranceSerialNo.value = newItem.custom_insurance_sr_no || ""
...
function updateItem() {
	...
	// Update insurance serial number if present
	if (localInsuranceSerialNo.value !== undefined) {
		updatedItem.custom_insurance_sr_no = localInsuranceSerialNo.value.trim() || null
	}
